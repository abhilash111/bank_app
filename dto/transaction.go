...
import "github.com/abhilash111/bank_app/errs"
...
func (r TransactionRequest) Validate() *errs.AppError {
	if !r.IsTransactionTypeWithdrawal() && !r.IsTransactionTypeDeposit() {
		return errs.NewUnExpectedError("Transaction type can only be deposit or withdrawal")
	}
	if r.Amount < 0 {
		return errs.NewUnExpectedError("Amount cannot be less than zero")
	}
	return nil
}
